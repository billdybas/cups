...
class Cups extends React.Component {
  ...
  handleFilterClick = async (drink) => {
    const count = await axios.get(
      `${config.API_URL}/cups.filter?drink=${drink}`
    );
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Main>
          <H1>
            Bill has drunk {count} cup{count !== 1 ? 's' : ''} of coffee ☕️
            this year.
          </H1>
          <H1>
            {filter &&
              `${filterCount} ${filterCount === 1 ? 'was' : 'were'} ${filter}.`}
          </H1>
        </Main>
        {drinks.length > 0 && <H3>Filter by drink</H3>}
        <Drinks>
          {drinks.map((drink, index) => (
            <Drink key={index} onClick={() => this.handleFilterClick(drink)}>
              {drink}
            </Drink>
          ))}
        </Drinks>
